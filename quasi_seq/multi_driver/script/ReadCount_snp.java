...
public class ReadCount_snp
{
    ...
    {
    	if (args.length < 1) {
			System.out.println("Usage: java ReadCount_snp xxxx.pileup.readcounts varFreq_threshold[default 0.01] coverage_threshold[default 8]");
		    System.exit(0);
    	}
    	int abiguity_threshold = 50;
    	int coverage_threshold = 8;
	double varFreq_threshold = 0.01;
    	if (args.length >1)
    		varFreq_threshold = Double.parseDouble(args[1]);
    	if (args.length >2)
					...
					if (field.length > 5)
					{
						...
						//float varFreq = new float;
							...
							if (varFreq > varFreq_threshold*100)
							{
								...
								//System.out.println(field[0]+"\t"+field[1]+"\t"+refAllele+"\t"+varAllele+"\t"+max_count+"\t"+second_max_count+"\t"+varFreq+"\t0.0001");
								snpFile.println(field[0]+"\t"+field[1]+"\t"+refAllele+"\t"+varAllele+"\t"+max_count+"\t"+second_max_count+"\t"+varFreq+"\t0.0001");
							}
						}
					}
				}
