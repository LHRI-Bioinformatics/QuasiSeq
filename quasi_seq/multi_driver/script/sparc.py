...
def pipeline(sampleID, reference, ranking_method, consensus_bul, *optionalfilters): #  must put the optional filters in string format
	...
	command = "blasr " + h5file + " " + filereference + " -minMatch 19 -nproc " + nproc + " -bestn 1 -unaligned "+ unalignedfile+ " -m 5 -out " +m5file
        print command
        subprocess.call(command, shell = True)

        command = "/opt/Sparc/compiled/Sparc b "+filereference+" m "+m5file+" k 1 g 1 c 2 t 0.1 o "+sample
        print command
        print >> sys.stderr
        subprocess.call(command, shell =True)	
	...
	print "fastq file:" + h5file  + "consensus file: " + consensusfile
	
	command = "blasr " + h5file + " " + consensusfile + " -nproc "+ nproc + " -bestn 1 -unaligned "+ unalignedfile+ " -sam -out " +samfile
	print command
    	subprocess.call(command, shell = True)
