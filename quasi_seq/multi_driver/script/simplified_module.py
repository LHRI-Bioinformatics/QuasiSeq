...
def pipeline(sampleID, reference, ranking_method, consensus_bul, *optionalfilters): #  must put the optional filters in string format
	...
	command = "blasr " + h5file + " " + filereference + " -nproc 10 -bestn 1 -unaligned "+ unalignedfile+ " -sam -out " +samfile 
	print command
	subprocess.call(command, shell = True)
		
	#command = "bwa mem -k17 -W40 -r10 -A1 -B1 -O1 -E1 -L0 -t 30 " + filereference + " " + h5file + " |samtools view -Sb - |samtools sort - -o " + bamfile
	command = "samtools view -Sb " + samfile+ " |samtools sort - " + sortfile
	...
	command = "samtools mpileup -BQ0 -d100000 -f"+" "+ filereference  + " " + bamfile +" > " + pileupfile
	print command 
	print >> sys.stderr
	subprocess.call(command, shell = True)
	print >> sys.stderr,"\n pileup_file is done!\n" + str(pileupfile) + "\n"
