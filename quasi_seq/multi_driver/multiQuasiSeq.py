		...
		with open(clusterFile,"r") as clusters: #where cluster output is the output of the cluster function
			...
			for line in clusters:
				...
				if os.path.isfile(currCluster):
					...
					p1.start()
					pool.append(p1)
					# pool.spawn(pipeline,outDir,currClusterFastq,consensusFile,currCluster,rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh,)
					#pool.spawn(pipeline,currClusterFastq,consensusFile,currCluster,rankingMethod, pool)
					#pool.apply_async(pipeline, args = (outDir,currClusterFastq,consensusFile,currCluster,rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, ))
					# pool.apply(pipeline, args = (outDir,currClusterFastq,consensusFile,currCluster,rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, ))
...
def main(argv):
   ...
   writeUnaligned = True
   # sampleName='pro4'
   # outDir='/data/quasiSeqOut/multiPro4/'
