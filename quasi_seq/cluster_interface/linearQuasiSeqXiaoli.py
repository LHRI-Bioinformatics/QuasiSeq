...
def getFullLengthReads(outDir,sampleName,m5File, start, end, buffer):
	...
	for line in m5:
		...
		if int(data[7])<=(start+buffer) and int(data[8])>=(end-buffer):
...
def alignFastqAndIndexSam_v1_3(fastq,reference,bamFile, *optionalfilters):
	...
	command = "blasr " + fastq + " " + reference + " -nproc 30 -bestn 1 -sam -out " +samFile + " -unaligned " + unaligned_reads
	print command
	print >> sys.stderr
...
def alignFastqBlasrM5(fastq,reference,m5File, *optionalfilters):
	start = time.time()
	command = "blasr " + fastq + " " + reference + " -minMatch 19 -nproc 30 -bestn 1 -m 5 -out " +m5File + " -unaligned " + ref_unaligned_reads
	print command
	print >> sys.stderr
	subprocess.call(command, shell = True,executable="/bin/bash")
	print >> sys.stderr, "\n m5File is done!\n" + str(m5File) +  "\n"
...
def normalizeStartEnd(consensusFile,referenceFile, startIndex, endIndex, *optionalfilters):
	...
	command = "blasr " + consensusFile + " " + referenceFile + " -minMatch 19 -nproc 30 -bestn 1 -m 1 -out " +m1File + " -unaligned "+ref_unaligned_reads
	print command
	print >> sys.stderr
	subprocess.call(command, shell = True,executable="/bin/bash")
	print >> sys.stderr, "\n m1File is done!\n" + str(m1File) +  "\n"
...
def pipeline(outDir, initialFastq, referenceFile, sampleName, rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh,minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, *optionalfilters):
	...
	unaligned_reads = outDir+str(sampleName) + "_unaligned.fa"
	ref_unaligned_reads = outDir+str(sampleName) + "ref_unaligned.fa"
...
def main(argv):
   start = time.time()
   pool = None #gevent.pool.Pool()
   queue = None #gevent.queue.Queue()
   clearing_final_fasta = open(("final_consensus.fasta"), "w")
   clearing_final_fasta.close()
   initialFastq = ''
   reference = ''
   sampleName = ''
   ...
   snpThresh=45
   try:
      opts, args = getopt.getopt(argv,"hb:r:s:m:",["initialFastq=","reference=","sampleName=","rankingMethod="])
   except getopt.GetoptError:
      print 'quasiSeq.py -b <initialFastq> -r <reference> -s <sampleName> -m <rankingMethod>'
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
         print 'quasiPipePacbioSubread.py -b <initialFastq> -r <reference> -s <sampleName> -m <rankingMethod>'
         sys.exit()
      elif opt in ("-b", "--initialFastq"):
         initialFastq = arg
      elif opt in ("-r", "--reference"):
         reference = arg
      elif opt in ("-s", "--sampleName"):
         sampleName = arg
      elif opt in ("-m", "--rankingMethod"):
         rankingMethod = arg
   pipeline(initialFastq, reference, sampleName, rankingMethod, pool, startIndex, endIndex, snpThresh)
   #pool.join()
   end = time.time()
