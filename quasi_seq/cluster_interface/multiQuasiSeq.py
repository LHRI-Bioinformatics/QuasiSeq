...
def alignFastqAndIndexSam_v1_3(fastq,reference,bamFile, *optionalfilters):
	...
	command = "blasr " + fastq + " " + reference + " -nproc "+numProcessors+" -bestn 1 -sam -out " +samFile
...
def alignFastqBlasrM5(fastq,reference,m5File, *optionalfilters):
	start = time.time()
	command = "blasr " + fastq + " " + reference + " -minMatch 19 -nproc "+numProcessors+" -bestn 1 -m 5 -out " +m5File
...
def normalizeStartEnd(consensusFile,referenceFile, startIndex, endIndex, *optionalfilters):
	...
	command = "blasr " + consensusFile + " " + referenceFile + " -minMatch 19 -nproc "+numProcessors+" -bestn 1 -m 1 -out " +m1File
		...
		with open(clusterFile,"r") as clusters: #where cluster output is the output of the cluster function
			...
			for line in clusters:
				...
				if os.path.isfile(currCluster):
					...
					print "!!!!!!!!!! Recursive call for "+currCluster+" using processCluster !!!!!!!!!!"
					# pipelineArgs=(outDir,currClusterFastq,consensusFile,currCluster,rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, )
					# p1 = Process(target=pipeline, args=pipelineArgs)
					processClusterArgs=(fastq,outDir,currClusterFastq,consensusFile,currCluster,rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, )
					p1 = Process(target=processCluster, args=processClusterArgs)
					p1.start()
					pool.append(p1)
					procs.append(p1)
...
def pipeline(outDir, initialFastq, referenceFile, sampleName, rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh,minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, *optionalfilters):
	...
	if startIndex==0 or startIndex==None:
		startIndex=1
		endIndex=getConsensusLength(referenceFile)
	else:
...
def startPipeline(outDir,initialFastq, referenceFile, sampleName, rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, currTaskId, *optionalfilters):
	...
	pipelineArgs=(outDir, fullLengthReadFastq, firstRoundConsensusFile, sampleName, rankingMethod, pool,startIndex,endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh, )
	p1 = Process(target=pipeline, args=pipelineArgs)
	p1.start()
	pool.append(p1)
	# pool.close()
...
def main(argv):
   ...
   # sampleName='pro19'
   # outDir='/data/quasiSeqOut/multiPro19/'
   # initialFastq='/data/pro19_subread.fastq'
   # ori_referenceFile='/data/HxB2.fasta'
   # referenceFile=outDir+'HxB2.fasta'
   # rankingMethod='percentage'
   # startIndex=800 #Pro4,Pro19
   # endIndex=7900 #Pro4,Pro19
   # snpThresh='40'
   # percentThresh=5.0
   ...
   outDir='/data/quasiSeqOut/multiCommandline/'
   sampleName='pro4sc_nonLTR_subread'
   initialFastq='/data/pro4sc_nonLTR_subread.fastq'
   ori_referenceFile='/data/HxB2.fasta'
   referenceFile=outDir+'HxB2.fasta'
   rankingMethod='percentage'
   startIndex=800 #Pro4,Pro19
   ...
   copyfile(ori_referenceFile, referenceFile)
