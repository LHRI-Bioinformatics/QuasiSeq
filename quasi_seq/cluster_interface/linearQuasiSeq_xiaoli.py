...
def alignFastqAndIndexSam_v1_3(fastq,reference,bamFile, *optionalfilters):
	...
	#command = "blasr " + fastq + " " + reference + " -nproc "+numProcessors+" -bestn 1 -sam -out " +samFile
        command = "blasr " + fastq + " " + reference + " -nproc 16 -bestn 1 -sam -out " +samFile
	if writeUnaligned:
...
def alignFastqBlasrM5(fastq,reference,m5File, *optionalfilters):
	...
	#command = "blasr " + fastq + " " + reference + " -minMatch 19 -nproc "+numProcessors+" -bestn 1 -m 5 -out " +m5File
        command = "blasr " + fastq + " " + reference + " -minMatch 19 -nproc 16 -bestn 1 -m 5 -out " +m5File
...
#command = "blasr " + consensusFile + " " + referenceFile + " -minMatch 19 -nproc "+numProcessors+" -bestn 1 -m 1 -out " +m1File
        command = "blasr " + consensusFile + " " + referenceFile + " -minMatch 19 -nproc 16 -bestn 1 -m 1 -out " +m1File
...
def startPipeline(outDir,initialFastq, referenceFile, sampleName, rankingMethod, pool, startIndex, endIndex, snpThresh, percentThresh, minorAlleleReadThresh, pvalThresh, maxSignatures, estimatedErrorRate, readCountSNPPvalThresh, readCountSNPVarFreqThresh, readCountSNPCoverageThresh,  *optionalfilters):
	print("This is linearQuasiSeq.py v1.3")
...
def main(argv):
   ...
   # outDir='/data/quasiSeqOut/linear/'
   # initialFastq='/data/lu10mix_FL_cc.fastq'
   # ori_referenceFile='/data/flu1PB.fa'
   # referenceFile=outDir+'flu1PB.fa'
   ...
   outDir='/data/quasiSeqOut/linearCommandline/'
